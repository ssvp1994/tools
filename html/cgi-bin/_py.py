...
def top_row(final): #naming the indexes of the file
    ...
    df4 = df.rename(columns={"Unnamed: 0": 'Number', "Unnamed: 1": 'Priority', "Unnamed: 2": 'Opened', "Unnamed: 3": 'Definition',
...
def combing_files(path,newfinal): #dumping the information from all the files
...
def drop_duplicates(final,keep): #Drops the duplicates
    xls_file = ExcelFile(final, index=False)
    df = xls_file.parse('Page 1')
    print("\nDropping duplicates")
    df4 = df.drop_duplicates(subset=['Number', 'Expert_Assigned', 'Opened', 'Definition', 'Value', 'Created'], keep=keep)
...
def end_time(file): #changing the end time in columns
    wb = xlwt.Workbook()
    outsheet = wb.add_sheet('Page 1', cell_overwrite_ok=True)
...
def res_row(file):#printing new rows whose facts were resolved for a time frame and later got reopened
    wb = xlwt.Workbook()
    outsheet = wb.add_sheet('Page 1', cell_overwrite_ok=True)
    open = xlrd.open_workbook(file)
    insheet = open.sheet_by_name("Page 1")
...
def correct_dept(newfinal): #removing the Expert names from the defination column
    wb = xlwt.Workbook()
    outsheet = wb.add_sheet('Page 1', cell_overwrite_ok=True)
...
def aggregate(file):
    ...
    for row in range(1, ((insheet.nrows)-1)):
        if insheet.cell_value(row,0)==insheet.cell_value(row+1,0) and insheet.cell_value(row,3)!="Resolved":
            ...
            total = sum(list1)
        elif insheet.cell_value(row, 0) != insheet.cell_value(row + 1, 0) and insheet.cell_value(row,3)!="Resolved" :
            list1.append(insheet.cell_value(row, 18))
...
def holidays(startdate,enddate):
    ...
    fedaral={42005:"New Year's Day",42023:"Birthday of Martin Luther King, Jr.", 42051:"Washington's Birthday",42149:"Memorial Day",
             42188:"Independence Day",42254:"Labor Day", 42289:"Columbus Day",42319:"Veterans Day",42334:"Thanksgiving Day",42363:"Christmas Day"}
    fedaral.update({42370:"New Year's Day",42387:"Birthday of Martin Luther King, Jr.",42415:"Washington's Birthday",42520:"Memorial Day",
                    42555:"Independence Day",42618:"Labor Day",42653:"Columbus Day",42685:"Veterans Day",42698:"Thanksgiving Day",42730:"Christmas Day"})
    fedaral.update({42737: "New Year's Day",42751:"Birthday of Martin Luther King, Jr.",42786:"Washington's Birthday",42884:"Memorial Day",42920:"Independence Day",42982:"Labor Day",
                    43017:"Columbus Day",43049:"Veterans Day",43062:"Thanksgiving Day",43094:"Christmas Day"})
    fedaral.update({43101:"New Year's Day",43115:"Birthday of Martin Luther King, Jr.",43150:"Washington's Birthday",43248:"Memorial Day",43285:"Independence Day",43346:"Labor Day",
...
def database_operations():
    fromdb = r"C:\Users\Administrator\Desktop\cgitest\cgitest\cgi-bin\downloads\fromdb.xls"  # path to folder to save from database
    ...
    def remove_common_data():
        new_data = r"C:\Users\Administrator\Desktop\cgitest\cgitest\cgi-bin\downloads\newcombined.xls"

        final = r"C:\Users\Administrator\Desktop\cgitest\cgitest\cgi-bin\downloads\New_data.xls"
        combined_New_fromdb = r"C:\Users\Administrator\Desktop\cgitest\cgitest\cgi-bin\downloads\New_data.xls"
        ...
        outsheet = wb.add_sheet('Page 1', cell_overwrite_ok=True)
        outrow=0
        for x in [fromdb,final]:
            open = xlrd.open_workbook(x)
            insheet = open.sheet_by_name("Page 1")
            for row in range(0, insheet.nrows):
                for col in range(0,insheet.ncols):
                    outsheet.write(outrow, col, insheet.cell_value(row, col))
                outrow+=1
        wb.save(combined_New_fromdb)



    def copy_to_db():
        file = r"C:\Users\Administrator\Desktop\cgitest\cgitest\cgi-bin\downloads\New_data.xls"

        sheet = xlrd.open_workbook(file).sheets()[0]

        print("\n\n Copying to database")

        query = "INSERT INTO dataware (Number,	Priority,	Opened,	Definition,	Value,	Expert_Assigned,	Created,	End_time,	Resolved,	New_Resolved,	Closed,	Main_file,	End_State,Formatted_Created,Formatted_End_time," \
